...
def main():
    ...
    # 録音の移動について:
    # SA権限ではユーザー所有フォルダからのファイル移動ができない(cannotAddParent)ため、
    # GAS経由の実行ではGAS側がディスパッチ成功時に移動済み（transcript_file_idが渡ってくる場合）。
    # 手動ディスパッチ等の旧フローのみ、ここで移動を試みる。
    if not args.transcript_file_id:
        try:
